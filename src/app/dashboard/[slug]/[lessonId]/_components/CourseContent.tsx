"use client";
import { LessonContentType } from "@/app/dataAcclyr/course/get-lesson-content";
...
export function CourseContent({ data }: iAppProps) {
    ...
    function VideoPlayer({thumbnailKey, videoKey }: {
        thumbnailKey: string;
        videoKey: string;
    }) {
        const videoUrl = useConstructImageUrl(videoKey);
        const thumbnailUrl = useConstructImageUrl(thumbnailKey);

        if (!videoKey) {
            return (
                <div className="aspect-video bg-muted rounded-lg flex flex-col items-center justify-center">
                    <BookIcon className="size-16 text-primary mx-auto mb-4" />
                    <p className="text-muted-foreground">This lesson video is not uploaded yet.</p>
                </div>
            )
        }
        ...
        return (
            <div className="aspect-video bg-black rounded-lg relative overflow-hidden">
                <video className="w-full h-full object-cover" controls poster={thumbnailUrl}>
                    ...
                    Your Browser does not suppot video format.
                </video>
            </div>
        )
    }
