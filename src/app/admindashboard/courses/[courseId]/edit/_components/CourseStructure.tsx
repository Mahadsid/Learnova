...
// for drag and drop functionality using "dnd kit" npm install @dnd-kit/core , also we want sortable content so another library npm install @dnd-kit/sortable check docs here https://docs.dndkit.com/presets/sortable
export function CourseStructure({ data }: iAppProps) {
    ...
    )) || []; //since when we have no course so it will have no chapters so we add || [] empty array so no errors. 
    ...
    function handleDragEnd(event : DragEndEvent) {
        ...
        if (activeType === 'chapter') {
            ...
            if (!targetChapterId) {
                toast.error("Could not determine the chapter for reodering.");
                return;
            }
            ...
            const updatedChapterForState = reorderLocalChapters.map((chapter, index) => ({
                ...chapter,
                order: index + 1,
            }));
        ...
        if (activeType === 'lesson' && overType === 'lesson') {
            ...
            if (!chapterId || chapterId !== overChapterId ) {
                toast.error(
                    "Invalid chapter ID : Lesson move between different chapters is not allowed."
                );
                return;
            }
